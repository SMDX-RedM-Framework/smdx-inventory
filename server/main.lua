...
local function CheckVersion()
    PerformHttpRequest('https://raw.githubusercontent.com/SMDX-RedM-Framework/smdx-inventory/main/version.txt', function(err, text, headers)
...
local ConvertQuality = function(item)
    ...
    local DecayRate = SMDXCore.Shared.Items[item.name:lower()]["decay"] ~= nil and SMDXCore.Shared.Items[item.name:lower()]["decay"] or 0.0
...
local function LoadInventory(source, citizenid)
    ...
    for _, item in pairs(inventory) do
        if item then
            local itemInfo = SMDXCore.Shared.Items[item.name:lower()]
            if itemInfo then
...
local function AddItem(source, item, amount, slot, info, created)
    ...
    local itemInfo = SMDXCore.Shared.Items[item:lower()]
    local time = os.time()

    if not itemInfo and not Player.Offline then
        SMDXCore.Functions.Notify(source, "Item does not exist", 'error')
        return false
    end
    ...
    if itemInfo['type'] == 'weapon' then
        info.serie = info.serie or tostring(SMDXCore.Shared.RandomInt(2) .. SMDXCore.Shared.RandomStr(3) .. SMDXCore.Shared.RandomInt(1) .. SMDXCore.Shared.RandomStr(2) .. SMDXCore.Shared.RandomInt(3) .. SMDXCore.Shared.RandomStr(4))
    ...
    if (totalWeight + (itemInfo['weight'] * amount)) <= Config.MaxInventoryWeight then
        if (slot and Player.PlayerData.items[slot]) and (Player.PlayerData.items[slot].name:lower() == item:lower()) and (itemInfo['type'] == 'item' and not itemInfo['unique']) then
            if Player.PlayerData.items[slot].info.quality == info.quality then
                ...
                TriggerEvent('smdx-log:server:CreateLog', 'playerinventory', 'AddItem', 'green', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** got item: [slot:' .. slot .. '], itemname: ' .. Player.PlayerData.items[slot].name .. ', added amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[slot].amount)
                return true
            else
                for i = 1, Config.MaxInventorySlots, 1 do
                    if Player.PlayerData.items[i] == nil then
                        ...
                        Player.Functions.SetPlayerData("items", Player.PlayerData.items)
                        if Player.Offline then return true end
                        TriggerEvent('smdx-log:server:CreateLog', 'playerinventory', 'AddItem', 'green', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** got item: [slot:' .. i .. '], itemname: ' .. Player.PlayerData.items[i].name .. ', added amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[i].amount)
                        return true
                    end
                end
            end
        ...
        elseif itemInfo['unique'] or (not slot or slot == nil) or itemInfo['type'] == 'weapon' then
                    for i = 1, Config.MaxInventorySlots, 1 do
                        ...
                        elseif Player.PlayerData.items[i] ~= nil and i == Config.MaxInventorySlots then
                            TriggerClientEvent('SMDXCore:Notify', source, "Inventory too full. Item was dropped on the ground.", 'error', 1000)
    ...
    if slot then
        if Player.PlayerData.items[slot].amount > amount then
            ...
            if not Player.Offline then
                TriggerEvent('smdx-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. slot .. '], itemname: ' .. Player.PlayerData.items[slot].name .. ', removed amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[slot].amount)
            end
        ...
        elseif Player.PlayerData.items[slot].amount == amount then
            ...
            TriggerEvent('smdx-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. slot .. '], itemname: ' .. item .. ', removed amount: ' .. amount .. ', item removed')
        ...
        for _, _slot in pairs(slots) do
            if Player.PlayerData.items[_slot].amount > amountToRemove then
                ...
                if not Player.Offline then
                    TriggerEvent('smdx-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. _slot .. '], itemname: ' .. Player.PlayerData.items[_slot].name .. ', removed amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[_slot].amount)
                end
            ...
            elseif Player.PlayerData.items[_slot].amount == amountToRemove then
                ...
                TriggerEvent('smdx-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. _slot .. '], itemname: ' .. item .. ', removed amount: ' .. amount .. ', item removed')
...
local function ClearInventory(source, filterItems)
    ...
    TriggerEvent('smdx-log:server:CreateLog', 'playerinventory', 'ClearInventory', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** inventory cleared')
end
...
local function SetInventory(source, items)
    ...
    TriggerEvent('smdx-log:server:CreateLog', 'playerinventory', 'SetInventory', 'blue', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** items set: ' .. json.encode(items))
end
...
local function CreateUsableItem(itemName, data)
    SMDXCore.Functions.CreateUseableItem(itemName, data)
end
...
local function GetUsableItem(itemName)
    return SMDXCore.Functions.CanUseItem(itemName)
end

exports("GetUsableItem", GetUsableItem)

---Use an item from the SMDXCore.UsableItems table if a callback is present
...
local function AddToStash(stashId, slot, otherslot, itemName, amount, info, created)
    amount = tonumber(amount) or 1
    local ItemData = SMDXCore.Shared.Items[itemName]
    if not ItemData.unique then
        if Stashes[stashId].items[slot] and Stashes[stashId].items[slot].name == itemName then
            ...
            local itemInfo = SMDXCore.Shared.Items[itemName:lower()]
...
local function CreateNewDrop(source, fromSlot, toSlot, itemAmount, created)
    ...
    if RemoveItem(source, itemData.name, itemAmount, itemData.slot) then
        ...
        local itemInfo = SMDXCore.Shared.Items[itemData.name:lower()]
        ...
        TriggerEvent("smdx-log:server:CreateLog", "drop", "New Item Drop", "red", "**".. GetPlayerName(source) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..source.."*) dropped new item; name: **"..itemData.name.."**, amount: **" .. itemAmount .. "**")
        ...
        SMDXCore.Functions.Notify(source, Lang:t("notify.missitem"), "error")
    end
end
...
AddEventHandler('SMDXCore:Server:PlayerLoaded', function(Player)
    SMDXCore.Functions.AddPlayerMethod(Player.PlayerData.source, "AddItem", function(item, amount, slot, info)
        return AddItem(Player.PlayerData.source, item, amount, slot, info)
    end)

    SMDXCore.Functions.AddPlayerMethod(Player.PlayerData.source, "RemoveItem", function(item, amount, slot)
        return RemoveItem(Player.PlayerData.source, item, amount, slot)
    end)

    SMDXCore.Functions.AddPlayerMethod(Player.PlayerData.source, "GetItemBySlot", function(slot)
        return GetItemBySlot(Player.PlayerData.source, slot)
    end)

    SMDXCore.Functions.AddPlayerMethod(Player.PlayerData.source, "GetItemByName", function(item)
        return GetItemByName(Player.PlayerData.source, item)
    end)

    SMDXCore.Functions.AddPlayerMethod(Player.PlayerData.source, "GetItemsByName", function(item)
        return GetItemsByName(Player.PlayerData.source, item)
    end)

    SMDXCore.Functions.AddPlayerMethod(Player.PlayerData.source, "ClearInventory", function(filterItems)
        ClearInventory(Player.PlayerData.source, filterItems)
    end)

    SMDXCore.Functions.AddPlayerMethod(Player.PlayerData.source, "SetInventory", function(items)
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    local Players = SMDXCore.Functions.GetSMDXPlayers()
    for k in pairs(Players) do
        SMDXCore.Functions.AddPlayerMethod(k, "AddItem", function(item, amount, slot, info)
            return AddItem(k, item, amount, slot, info)
        end)

        SMDXCore.Functions.AddPlayerMethod(k, "RemoveItem", function(item, amount, slot)
            return RemoveItem(k, item, amount, slot)
        end)

        SMDXCore.Functions.AddPlayerMethod(k, "GetItemBySlot", function(slot)
            return GetItemBySlot(k, slot)
        end)

        SMDXCore.Functions.AddPlayerMethod(k, "GetItemByName", function(item)
            return GetItemByName(k, item)
        end)

        SMDXCore.Functions.AddPlayerMethod(k, "GetItemsByName", function(item)
            return GetItemsByName(k, item)
        end)

        SMDXCore.Functions.AddPlayerMethod(k, "ClearInventory", function(filterItems)
            ClearInventory(k, filterItems)
        end)

        SMDXCore.Functions.AddPlayerMethod(k, "SetInventory", function(items)
            SetInventory(k, items)
        end)
    end
end)

RegisterNetEvent('SMDXCore:Server:UpdateObject', function()
...
RegisterNetEvent('inventory:server:combineItem', function(item, fromItem, toItem)
    ...
    local recipe = SMDXCore.Shared.Items[toItem.name].combinable

    if recipe and recipe.reward ~= item then return end
    if not recipeContains(recipe, fromItem) then return end

    TriggerClientEvent('inventory:client:ItemBox', src, SMDXCore.Shared.Items[item], 'add')
    AddItem(src, item, 1)
    ...
    if not ply.state.inv_busy then
        if name and id then
            ...
            if name == "stash" then
                if Stashes[id] then
                    if Stashes[id].isOpen then
                        local Target = SMDXCore.Functions.GetPlayer(Stashes[id].isOpen)
                        if Target then
            ...
            elseif name == "otherplayer" then
                local OtherPlayer = SMDXCore.Functions.GetPlayer(tonumber(id))
                if OtherPlayer then
                ...
                if Drops[id] then
                    if Drops[id].isOpen then
                        local Target = SMDXCore.Functions.GetPlayer(Drops[id].isOpen)
                        if Target then
            ...
            TriggerClientEvent("smdx-inventory:client:closeinv", id)
            TriggerClientEvent("inventory:client:OpenInventory", src, {}, Player.PlayerData.items, secondInv)
        else
            TriggerClientEvent("inventory:client:OpenInventory", src, {}, Player.PlayerData.items)
        end
    else
        SMDXCore.Functions.Notify(src, Lang:t("notify.noaccess"), 'error')
    end
end)
...
RegisterNetEvent('inventory:server:UseItemSlot', function(slot)
    ...
    if not itemData then return end
    local itemInfo = SMDXCore.Shared.Items[itemData.name]
    if itemData.type == "weapon" then
        TriggerClientEvent("smdx-weapons:client:UseWeapon", src, itemData, itemData.info.quality and itemData.info.quality > 0)
    ...
    elseif itemData.useable then
        if itemData.info.quality > 0 then
            ...
            if itemInfo['delete'] and RemoveItem(src, itemData.name, 1, slot) then
                ...
                SMDXCore.Functions.Notify(src, SMDXCore.Shared.Items[itemData.name].label..' has decayed and has been removed!', 'error')
            else
                SMDXCore.Functions.Notify(src, 'You can\'t use this item!', 'error')
            end
        end
    end
end)
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
    ...
    if (fromInventory == "player" or fromInventory == "hotbar") and (SMDXCore.Shared.SplitStr(toInventory, "-")[1] == "itemshop") then
        return
    end
    ...
    if fromInventory == "player" or fromInventory == "hotbar" then
        ...
        if fromItemData and fromItemData.amount >= fromAmount then
            ...
            elseif SMDXCore.Shared.SplitStr(toInventory, "-")[1] == "otherplayer" then
                local playerId = tonumber(SMDXCore.Shared.SplitStr(toInventory, "-")[2])
                local OtherPlayer = SMDXCore.Functions.GetPlayer(playerId)
                ...
                if toItemData then
                    --Player.PlayerData.items[fromSlot] = toItemData
                    local itemInfo = SMDXCore.Shared.Items[toItemData.name:lower()]
                    toAmount = tonumber(toAmount) or toItemData.amount
                    if toItemData.name ~= fromItemData.name then
                        RemoveItem(playerId, itemInfo["name"], toAmount, fromSlot)
                        AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
                        TriggerEvent("smdx-log:server:CreateLog", "robbing", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount.. "** with player: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | id: *"..OtherPlayer.PlayerData.source.."*)")
                    end
                else
                    local itemInfo = SMDXCore.Shared.Items[fromItemData.name:lower()]
                    TriggerEvent("smdx-log:server:CreateLog", "robbing", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** to player: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | id: *"..OtherPlayer.PlayerData.source.."*)")
                end
                local itemInfo = SMDXCore.Shared.Items[fromItemData.name:lower()]
                AddItem(playerId, itemInfo["name"], fromAmount, toSlot, fromItemData.info, itemInfo["created"])
            elseif SMDXCore.Shared.SplitStr(toInventory, "-")[1] == "stash" then
                local stashId = SMDXCore.Shared.SplitStr(toInventory, "-")[2]
                ...
                if toItemData then
                    ...
                    if toItemData.name ~= fromItemData.name then
                        ...
                        AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
                        TriggerEvent("smdx-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - stash: *" .. stashId .. "*")
                    end
                else
                    local itemInfo = SMDXCore.Shared.Items[fromItemData.name:lower()]
                    TriggerEvent("smdx-log:server:CreateLog", "stash", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - stash: *" .. stashId .. "*")
                end
                local itemInfo = SMDXCore.Shared.Items[fromItemData.name:lower()]
                ...
                if toInventory == nil or toInventory == 0 then
                    ...
                    if toItemData then
                        ...
                        if toItemData.name ~= fromItemData.name then
                            ...
                            TriggerEvent("smdx-log:server:CreateLog", "drop", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - dropid: *" .. toInventory .. "*")
                        end
                    else
                        local itemInfo = SMDXCore.Shared.Items[fromItemData.name:lower()]
                        TriggerEvent("smdx-log:server:CreateLog", "drop", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - dropid: *" .. toInventory .. "*")
                    end
                    local itemInfo = SMDXCore.Shared.Items[fromItemData.name:lower()]
            ...
            SMDXCore.Functions.Notify(src, Lang:t("notify.missitem"), "error")
        end
    elseif SMDXCore.Shared.SplitStr(fromInventory, "-")[1] == "otherplayer" then
        local playerId = tonumber(SMDXCore.Shared.SplitStr(fromInventory, "-")[2])
        local OtherPlayer = SMDXCore.Functions.GetPlayer(playerId)
        ...
        if fromItemData and fromItemData.amount >= fromAmount then
            ...
            if toInventory == "player" or toInventory == "hotbar" then
                ...
                if toItemData then
                    itemInfo = SMDXCore.Shared.Items[toItemData.name:lower()]
                    toAmount = tonumber(toAmount) or toItemData.amount
                    if toItemData.name ~= fromItemData.name then
                        RemoveItem(src, toItemData.name, toAmount, toSlot)
                ...
                itemInfo = SMDXCore.Shared.Items[fromItemData.name:lower()]
                AddItem(playerId, itemInfo["name"], fromAmount, toSlot, fromItemData.info)
            end
        else
            SMDXCore.Functions.Notify(src, "Item doesn't exist", "error")
        end
    elseif SMDXCore.Shared.SplitStr(fromInventory, "-")[1] == "stash" then
        local stashId = SMDXCore.Shared.SplitStr(fromInventory, "-")[2]
        ...
        if fromItemData and fromItemData.amount >= fromAmount then
            ...
            if toInventory == "player" or toInventory == "hotbar" then
                ...
                if toItemData then
                    ...
                    if toItemData.name ~= fromItemData.name then
                        ...
                        TriggerEvent("smdx-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** stash: *" .. stashId .. "*")
                    else
                        TriggerEvent("smdx-log:server:CreateLog", "stash", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from stash: *" .. stashId .. "*")
                    end
                else
                    TriggerEvent("smdx-log:server:CreateLog", "stash", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** stash: *" .. stashId .. "*")
                end
            ...
            SMDXCore.Functions.Notify(src, Lang:t("notify.itemexist"), "error")
        end
    elseif SMDXCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
        local shopType = SMDXCore.Shared.SplitStr(fromInventory, "-")[2]
        local itemData = ShopItems[shopType].items[fromSlot]
        local itemInfo = SMDXCore.Shared.Items[itemData.name:lower()]
        local bankBalance = Player.PlayerData.money["bank"]
        local price = tonumber((itemData.price*fromAmount))

        if SMDXCore.Shared.SplitStr(shopType, "_")[1] == "Dealer" then
            if SMDXCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
                price = tonumber(itemData.price)
                if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
                    itemData.info.serie = tostring(SMDXCore.Shared.RandomInt(2) .. SMDXCore.Shared.RandomStr(3) .. SMDXCore.Shared.RandomInt(1) .. SMDXCore.Shared.RandomStr(2) .. SMDXCore.Shared.RandomInt(3) .. SMDXCore.Shared.RandomStr(4))
                    itemData.info.quality = itemData.info.quality or 100
                    AddItem(src, itemData.name, 1, toSlot, itemData.info)
                    TriggerClientEvent('smdx-drugs:client:updateDealerItems', src, itemData, 1)
                    SMDXCore.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
                    TriggerEvent("smdx-log:server:CreateLog", "dealers", "Dealer item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
                else
                    SMDXCore.Functions.Notify(src, Lang:t("notify.notencash"), "error")
                end
            else
                if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
                    itemData.info.quality = itemData.info.quality or 100
                    AddItem(src, itemData.name, fromAmount, toSlot, itemData.info)
                    TriggerClientEvent('smdx-drugs:client:updateDealerItems', src, itemData, fromAmount)
                    SMDXCore.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
                    TriggerEvent("smdx-log:server:CreateLog", "dealers", "Dealer item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. "  for $"..price)
                else
                    SMDXCore.Functions.Notify(src, "You don't have enough cash..", "error")
                end
            end
        elseif SMDXCore.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
            if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
                ...
                itemData.info.quality = itemData.info.quality or 100
                AddItem(src, itemData.name, fromAmount, toSlot, itemData.info)
                TriggerClientEvent('smdx-shops:client:UpdateShop', src, SMDXCore.Shared.SplitStr(shopType, "_")[2], itemData, fromAmount)
                SMDXCore.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
                TriggerEvent("smdx-log:server:CreateLog", "shops", "Shop item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
            ...
            if toInventory == "player" or toInventory == "hotbar" then
                ...
                if toItemData then
                    ...
                    if toItemData.name ~= fromItemData.name then
                        ...
                        TriggerEvent("smdx-log:server:CreateLog", "drop", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** - dropid: *" .. fromInventory .. "*")
                    else
                        TriggerEvent("smdx-log:server:CreateLog", "drop", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** - from dropid: *" .. fromInventory .. "*")
                    end
                else
                    TriggerEvent("smdx-log:server:CreateLog", "drop", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** -  dropid: *" .. fromInventory .. "*")
                end
                AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info, fromItemData["created"])
            else
                toInventory = tonumber(toInventory)
...
RegisterNetEvent('smdx-inventory:server:SaveStashItems', function(stashId, items)
    MySQL.insert('INSERT INTO stashitems (stash, items) VALUES (:stash, :items) ON DUPLICATE KEY UPDATE items = :items', {
        ['stash'] = stashId,
        ['items'] = json.encode(items)
    })
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot)
    ...
    local OtherPlayer = SMDXCore.Functions.GetPlayer(target)
    local dist = #(GetEntityCoords(GetPlayerPed(src))-GetEntityCoords(GetPlayerPed(target)))
    if Player == OtherPlayer then return SMDXCore.Functions.Notify(src, Lang:t("notify.gsitem")) end
    if dist > 2 then return SMDXCore.Functions.Notify(src, Lang:t("notify.tftgitem")) end
    local item = GetItemBySlot(src, slot)
    if not item then SMDXCore.Functions.Notify(src, Lang:t("notify.infound")); return end
    if item.name ~= name then SMDXCore.Functions.Notify(src, Lang:t("notify.iifound")); return end
    ...
    if amount <= item.amount then
        ...
        if RemoveItem(src, item.name, amount, item.slot) then
            if AddItem(target, item.name, amount, false, item.info, item.created) then
                ...
                TriggerClientEvent('inventory:client:ItemBox',target, SMDXCore.Shared.Items[item.name], "add")
                SMDXCore.Functions.Notify(target, Lang:t("notify.gitemrec")..amount..' '..item.label..Lang:t("notify.gitemfrom")..Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname)
                TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, true)
                TriggerClientEvent('inventory:client:ItemBox',src, SMDXCore.Shared.Items[item.name], "remove")
                SMDXCore.Functions.Notify(src, Lang:t("notify.gitemyg") .. OtherPlayer.PlayerData.charinfo.firstname.." "..OtherPlayer.PlayerData.charinfo.lastname.. " " .. amount .. " " .. item.label .."!")
                TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
                TriggerClientEvent('smdx-inventory:client:giveAnim', src)
                TriggerClientEvent('smdx-inventory:client:giveAnim', target)
            else
                AddItem(src, item.name, amount, item.slot, item.info, item.created)
                SMDXCore.Functions.Notify(src, Lang:t("notify.gitinvfull"), "error")
                SMDXCore.Functions.Notify(target, Lang:t("notify.giymif"), "error")
                TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, false)
                TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, false)
            end
        else
            SMDXCore.Functions.Notify(src, Lang:t("notify.gitydhei"), "error")
        end
    else
        SMDXCore.Functions.Notify(src, Lang:t("notify.gitydhitt"))
    end
end)
...
SMDXCore.Functions.CreateCallback('smdx-inventory:server:GetStashItems', function(_, cb, stashId)
    cb(GetStashItems(stashId))
end)

SMDXCore.Functions.CreateCallback('inventory:server:GetCurrentDrops', function(_, cb)
    cb(Drops)
end)

SMDXCore.Functions.CreateCallback('SMDXCore:HasItem', function(source, cb, items, amount)
    print("^3SMDXCore:HasItem is deprecated, please use SMDXCore.Functions.HasItem, it can be used on both server- and client-side and uses the same arguments.^0")
    local retval = false
    local Player = SMDXCore.Functions.GetPlayer(source)
...
-- Decay System
SMDXCore.Functions.CreateCallback('inventory:server:ConvertQuality', function(source, cb, inventory, other)
    local src = source
    local data = {}
    local Player = SMDXCore.Functions.GetPlayer(src)
    ...
    for _, item in pairs(inventory) do
        if item.created then
            if SMDXCore.Shared.Items[item.name:lower()]["decay"] ~= nil or SMDXCore.Shared.Items[item.name:lower()]["decay"] ~= 0 then
                if item.info then
    ...
    if other then
        local inventoryType = SMDXCore.Shared.SplitStr(other.name, "-")[1]
        local uniqueId = SMDXCore.Shared.SplitStr(other.name, "-")[2]
...
SMDXCore.Commands.Add("resetinv", "Reset Inventory (Admin Only)", {{name="type", help="stash/trunk/glovebox"},{name="id/plate", help="ID of stash or license plate"}}, true, function(source, args)
    ...
    if invType and invId then
        ...
        elseif invType == "stash" then
            ...
            SMDXCore.Functions.Notify(source,  Lang:t("notify.navt"), "error")
        end
    else
        SMDXCore.Functions.Notify(source,  Lang:t("notify.anfoc"), "error")
    end
end, "admin")

SMDXCore.Commands.Add("rob", "Rob Player", {}, false, function(source, _)
    TriggerClientEvent("smdx-looting:client:RobPlayer", source)
end)

SMDXCore.Commands.Add("giveitem", "Give An Item (Admin Only)", {{name="id", help="Player ID"},{name="item", help="Name of the item (not a label)"}, {name="amount", help="Amount of items"}}, false, function(source, args)
    local id = tonumber(args[1])
    local Player = SMDXCore.Functions.GetPlayer(id)
    local amount = tonumber(args[3]) or 1
    local itemData = SMDXCore.Shared.Items[tostring(args[2]):lower()]
    if Player then
            if itemData then
                -- check iteminfo
                local info = {}
                ...
                elseif itemData["type"] == "weapon" then
                    amount = 1
                    info.serie = tostring(SMDXCore.Shared.RandomInt(2) .. SMDXCore.Shared.RandomStr(3) .. SMDXCore.Shared.RandomInt(1) .. SMDXCore.Shared.RandomStr(2) .. SMDXCore.Shared.RandomInt(3) .. SMDXCore.Shared.RandomStr(4))
                    info.quality = 100
                ...
                elseif itemData["name"] == "labkey" then
                    info.lab = exports["smdx-methlab"]:GenerateRandomLab()
                elseif itemData["name"] == "printerdocument" then
                    info.url = "https://cdn.discordapp.com/attachments/870094209783308299/870104331142189126/Logo_-_Display_Picture_-_Stylized_-_Red.png"
                elseif SMDXCore.Shared.Items[itemData["name"]]["decay"] and SMDXCore.Shared.Items[itemData["name"]]["decay"] > 0 then
                    info.quality = 100
                end

                if AddItem(id, itemData["name"], amount, false, info) then
                    SMDXCore.Functions.Notify(source, Lang:t("notify.yhg") ..GetPlayerName(id).." "..amount.." "..itemData["label"].. "", "success")
                else
                    SMDXCore.Functions.Notify(source,  Lang:t("notify.cgitem"), "error")
                end
            else
                SMDXCore.Functions.Notify(source,  Lang:t("notify.idne"), "error")
            end
    else
        SMDXCore.Functions.Notify(source,  Lang:t("notify.pdne"), "error")
    end
end, "admin")

SMDXCore.Commands.Add("randomitems", "Give Random Items (God Only)", {}, false, function(source, _)
    local filteredItems = {}
    for k, v in pairs(SMDXCore.Shared.Items) do
        if SMDXCore.Shared.Items[k]["type"] ~= "weapon" then
    ...
    for _ = 1, 10, 1 do
        ...
        if AddItem(source, randitem["name"], amount) then
            TriggerClientEvent('inventory:client:ItemBox', source, SMDXCore.Shared.Items[randitem["name"]], 'add')
            Wait(500)
        end
    end
end, "god")

SMDXCore.Commands.Add('clearinv', 'Clear Players Inventory (Admin Only)', { { name = 'id', help = 'Player ID' } }, false, function(source, args)
    local playerId = args[1] ~= '' and tonumber(args[1]) or source
    local Player = SMDXCore.Functions.GetPlayer(playerId)
    if Player then
        ClearInventory(playerId)
    else
        SMDXCore.Functions.Notify(source, "Player not online", 'error')
    end
end, 'admin')
...
CreateUsableItem("driver_license", function(source, item)
    ...
    local players = SMDXCore.Functions.GetPlayers()
...
RegisterNetEvent('inventory:server:dropOnGround', function(thissource, itemInfos, amount, info)
    ...
    TriggerEvent("smdx-log:server:CreateLog", "drop", "New Item Drop", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemData.name.."**, amount: **" .. itemAmount .. "**")
    TriggerClientEvent("inventory:client:DropItemAnim", source)
