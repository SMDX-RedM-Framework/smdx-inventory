...
local function CreateItemDrop(index)
    ...
    if Config.UseTarget then
        exports['smdx-target']:AddTargetEntity(dropItem, {
            options = {
                {
...
RegisterNetEvent('SMDXCore:Client:OnPlayerLoaded', function()
    ...
    SMDXCore.Functions.TriggerCallback("inventory:server:GetCurrentDrops", function(theDrops)
        Drops = theDrops
    end)
end)
...
RegisterNetEvent('SMDXCore:Client:UpdateObject', function()
    SMDXCore = exports['smdx-core']:GetCoreObject()
end)
...
RegisterNetEvent("smdx-inventory:client:closeinv", function()
    closeInventory()
end)

RegisterNetEvent('inventory:client:CheckOpenState', function(type, id, label)
    local name = SMDXCore.Shared.SplitStr(label, "-")[2]
    if type == "stash" then
    ...
    if bool then
        for k in pairs(items) do
            itemTable[#itemTable+1] = {
                item = items[k].name,
                label = SMDXCore.Shared.Items[items[k].name]["label"],
                image = items[k].image,
            }
        end
    end
...
RegisterNetEvent('inventory:client:OpenInventory', function(PlayerAmmo, inventory, other)
    ...
    SMDXCore.Functions.TriggerCallback('inventory:server:ConvertQuality', function(data)
...
RegisterNetEvent('smdx-inventory:client:giveAnim', function()
...
RegisterNUICallback('Notify', function(data, cb)
    SMDXCore.Functions.Notify(data.message, data.type)
    cb('ok')
end)

RegisterNUICallback('getCombineItem', function(data, cb)
    cb(SMDXCore.Shared.Items[data.item])
end)
...
RegisterNUICallback('combineWithAnim', function(data, cb)
    ...
    SMDXCore.Functions.Progressbar("combine_anim", animText, animTimeout, false, true, {
        ...
        StopAnimTask(ped, aDict, aLib, 1.0)
        SMDXCore.Functions.Notify(Lang:t("notify.failed"), "error")
    end)
    cb('ok')
end)
...
RegisterNUICallback("GiveItem", function(data, cb)
    local player, distance = SMDXCore.Functions.GetClosestPlayer(GetEntityCoords(PlayerPedId()))
    if player ~= -1 and distance < 3 then
        if data.inventory == 'player' then
            ...
            SMDXCore.Functions.Notify(Lang:t("notify.notowned"), "error")
        end
    else
        SMDXCore.Functions.Notify(Lang:t("notify.nonb"), "error")
    end
    cb('ok')
end)
...
CreateThread(function()
    while true do
        Wait(0)
        if IsControlJustReleased(0, SMDXCore.Shared.Keybinds['I']) then -- key open inventory I
...
CreateThread(function()
    while true do
        Wait(0)
        DisableControlAction(0, SMDXCore.Shared.Keybinds['1'])
        DisableControlAction(0, SMDXCore.Shared.Keybinds['2'])
        DisableControlAction(0, SMDXCore.Shared.Keybinds['3'])
        DisableControlAction(0, SMDXCore.Shared.Keybinds['4'])
        DisableControlAction(0, SMDXCore.Shared.Keybinds['5'])
        DisableControlAction(0, SMDXCore.Shared.Keybinds['Z'])
        if IsDisabledControlPressed(0, SMDXCore.Shared.Keybinds['1']) and IsInputDisabled(0) then  -- 1  slot
        ...
        if IsDisabledControlPressed(0, SMDXCore.Shared.Keybinds['2']) and IsInputDisabled(0) then  -- 2 slot
            if not PlayerData.metadata["isdead"] and not PlayerData.metadata["ishandcuffed"] then
                TriggerServerEvent("inventory:server:UseItemSlot", 2)
            end
        end

        if IsDisabledControlPressed(0, SMDXCore.Shared.Keybinds['3']) and IsInputDisabled(0) then -- 3 slot
            if not PlayerData.metadata["isdead"] and not PlayerData.metadata["ishandcuffed"] then
                TriggerServerEvent("inventory:server:UseItemSlot", 3)
            end
        end

        if IsDisabledControlPressed(0, SMDXCore.Shared.Keybinds['4']) and IsInputDisabled(0) then  -- 4 slot
            if not PlayerData.metadata["isdead"] and not PlayerData.metadata["ishandcuffed"] then
                TriggerServerEvent("inventory:server:UseItemSlot", 4)
            end
        end

        if IsDisabledControlPressed(0, SMDXCore.Shared.Keybinds['5']) and IsInputDisabled(0) then -- 5 slot
            if not PlayerData.metadata["isdead"] and not PlayerData.metadata["ishandcuffed"] then
                TriggerServerEvent("inventory:server:UseItemSlot", 5)
            end
        end

        if IsDisabledControlJustPressed(0, SMDXCore.Shared.Keybinds['Z']) and IsInputDisabled(0) then -- z  Hotbar
            isHotbar = not isHotbar
            ToggleHotbar(isHotbar)
        end
    end
